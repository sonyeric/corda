...
class P2PMessagingTest : NodeBasedTest() {
    ...
    private fun assertAllNodesAreUsed(distributedService: List<Node>, serviceName: String, commmunicator: Node) {
        // Setup each node in the distributed service to return back it's NodeInfo so that we can know which node is being used
        distributedService.forEach { node ->
            node.respondWith(node.info)
        }
        val serviceAddress = commmunicator.services.networkMapCache.run {
            commmunicator.net.getAddressOfParty(getPartyInfo(getNotary(serviceName)!!)!!)
        }
        ...
        for (it in 1..distributedService.size * 3) {
            participatingNodes += commmunicator.receiveFrom(serviceAddress).getOrThrow(10.seconds)
            if (participatingNodes.size == distributedService.size) {
                break
            }
        }
        assertThat(participatingNodes).containsOnlyElementsOf(distributedService.map(Node::info))
    }
